...
- **Rich menu**: `richmenu.js` (6-cell object + `setupRichMenu()`), `assets/richmenu.svg` (2500×1686 design — rasterize to `assets/richmenu.png`), `npm run setup:richmenu` uploads + sets default. **`preview.html`** renders the whole conversation (hero / carousel / cards / quick-reply / rich menu) in-browser for a LINE-less design review — open it directly.
- No tests/lint/build. `.env` + `assets/richmenu.png` are gitignored. `〔要確認〕` values (esp. 代表電話) in `faq.js` must be replaced with official info. See `martialarts-line-bot/README.md` (Japanese).
