...
├── pension-planning-kit/     # Zero-dep single-HTML 年金×ライフプランニング資料 (client-facing A4 brochure built on 日本年金機構「ねんきんネット」) → 3-step 現状把握→試算→プラン, print/PDF, in-browser editable (contenteditable + localStorage), JP UI
...
- **Everything lives in `pension-planning-kit/index.html`** (markup + CSS + JS in one file). 8 A4 `.page`s (表紙 → なぜ今 → 3ステップ → STEP1 → STEP2 → STEP3 → Q&A → チェックリスト+CTA); output via `@media print` (A4 = 794×1123px @96dpi), export = browser print dialog → "Save as PDF".
...
- **Facts discipline**: year-sensitive pension/tax figures (繰上げ0.4%減・繰下げ0.7%増/最大84%・iDeCo/NISA枠 等) are marked `〔要確認〕` inline, with a disclaimer on every page footer + P8 (一般的情報提供であり個別助言ではない). Update against 日本年金機構/金融庁/国税庁 before distributing. No tests/lint/build. See `pension-planning-kit/README.md` (Japanese).
