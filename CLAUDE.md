...
- **`index.html`** is the tool: score each person on **6 axes (0–5)** — ①実績 ②統率 ③育成 ④当事者 ⑤胆力 ⑥定着 — and `recommend()` maps the scores to a role (代表格 / プレイングリーダー / メンター / 管理No.2 / 育成トレーナー / 2番手候補 / プレイヤー) via an ordered priority table. Renders an **org chart + member cards**, add/edit via modal, persists to **localStorage** (`martialarts_1on1_board_v1`), JSON import/export, print/PDF. **7 営業部 members are seeded** as defaults (海音/酒井/上田/坂本るい/犬塚篤博/小林駿/菊池; 赤西 is a different company and is excluded; 犬塚 篤博＝アツヒロ is one person — surname 犬塚, given name 篤博). 海音 is the only member classified 代表格 (highest 当事者意識/野心 + top実績).
- **The `recommend()` logic in `index.html` and the 対応表 in `組織設計_来期営業部.md` must stay in sync** — edit both together. Deep-dives: `分析_酒井かいと_20260727.md` (トップセールス), `分析_小林駿_20260724.md` (縁の下の支援スペシャリスト志向＋退職意向＝定着リスク; supersedes the earlier 伝聞「2番手候補 シュン」framing), `分析_坂本るい.md` (係長代理→係長候補・上田課長の右腕/現場リーダー; 自ら昇進を望む), `分析_海音.md` (★代表格最有力・組織TOP志向), `分析_菊池.md` (仕組み・ツール＋礼儀礼節教育の支援). `組織設計_来期営業部.md` also carries a 教育・役割分担マップ (新人教育=犬塚 / アポ現場=坂本 / 礼儀礼節=菊池 / 全体リーダー=海音 / 数字=犬塚・小林).
