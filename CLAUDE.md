...
├── sourcing-handover/       # Zero-dep single-HTML 仕入れ引き継ぎコンソール (9/24 引き継ぎ用): 61問の質問シート + 物件出し実績のファネル集計 + マニュアル反映用Markdown生成, localStorage (JP UI)
...
├── cashflow-dashboard/       # Zero-dep single-HTML 資金繰りダッシュボード: drop a bank 取引明細 CSV (Shift_JIS/UTF-8 auto-detected) → 残高推移/月次収支/カテゴリ別支出/取引先ランキング/アラート + filterable 明細. All in-browser (no network); 明細は既定で保存しない (JP UI)
...
- **`index.html`** is the tool: score each person on **6 axes (0–5)** — ①実績 ②統率 ③育成 ④当事者 ⑤胆力 ⑥定着 — and `recommend()` maps the scores to a role (代表格 / プレイングリーダー / メンター / 管理No.2 / 育成トレーナー / 2番手候補 / プレイヤー) via an ordered priority table. Renders an **org chart + member cards**, add/edit via modal, persists to **localStorage** (`martialarts_1on1_board_v1`), JSON import/export, print/PDF. **6 営業部 members are seeded** as defaults (酒井海音/上田/坂本るい/犬塚篤博/小林駿/菊池; 赤西 is a different company and is excluded). Two same-person merges: 犬塚 篤博＝アツヒロ (surname 犬塚, given 篤博), and 酒井かいと＝海音 (海音 is the kanji for his given name かいと — earlier double-listed, now merged). 酒井 海音 is the only 代表格 (top実績＋当事者意識/野心), confirmed チームリーダー by the 8/12 提案書.
- **The `recommend()` logic in `index.html` and the 対応表 in `組織設計_来期営業部.md` must stay in sync** — edit both together. The authoritative source is `組織改編提案書_20260812.md` (CEO 長谷川's official proposal; board + 組織設計 are kept consistent with it). Deep-dives: `分析_酒井かいと_20260727.md` (★代表格・組織TOP志向＝チームリーダー確定; 海音回答を統合), `分析_小林駿_20260724.md` (縁の下の支援スペシャリスト志向＋退職意向＝定着リスク; supersedes the earlier 伝聞「2番手候補 シュン」framing), `分析_坂本るい.md` (係長代理→係長候補・上田課長の右腕/アポ現場教育; 自ら昇進を望む), `分析_菊池.md` (仕組み・ツール＋礼儀礼節教育の支援). `組織設計_来期営業部.md` also carries a 教育・役割分担マップ (新人教育=犬塚 / アポ現場=坂本 / 礼儀礼節=菊池 / 全体リーダー=酒井海音 / 数字=犬塚・小林).
...
Zero-dependency single-file vanilla-JS app (no build step — same family as `echo-interview-console`/`construction-book`). An **interactive financial-literacy self-check + study tool for 不動産経営者/営業** (defaults 株式会社 Martial Arts), built by merging two handout materials: a **50-question quiz (6 chapters)** and its companion **「金融リテラシー実践的ガイド」**. Browser quiz → **auto-scoring** → **章別スコア** → **per-question explanations**, plus a 学習(資料編) section rendering the guide's 6 themes.

- Run: `cd money-basics-quiz && python3 -m http.server 5186` → http://localhost:5186 (no install/build); opening `index.html` directly also works (fully client-side).
- **Everything lives in `money-basics-quiz/index.html`** (markup + CSS + JS). **`QUIZ` is the single source of truth** for questions: `{ch, q, text, opts:[4], ans:0..3 (a/b/c/d), exp}` — edit only here to add/fix items. `CHAPTERS` = chapter meta; `STUDY` = the 6 資料編 themes. Scoring/breakdown/review/`rankOf` (S/A/B/C/D) are pure functions; last result persists to **localStorage** (`ma_money_quiz_v1`) and shows on home. Modes: 全50問 / ランダム10問 (`shuffle`) / 章別, plus a **即時採点 (instant)** toggle (reveal per answer) vs test mode (score at end). Result sheet is print/PDF-ready (A4).
- **Facts discipline**: questions/answers/statistics transcribed faithfully from the handouts; **the answer key was verified to match all 50 items**. Two source inconsistencies are resolved in favor of the answer key and **annotated in 資料編**: Q34 (日米金利差縮小→円相場: key=円高進行 / guide body=安定傾向) and Q45 (収支把握の推奨期間: key=1ヶ月 / guide=1週間). 税率・基礎控除・NISA/iDeCo枠・インフレ率 are 改正/変動する参考値 (Q40 NISA枠 annotated) — verify against 国税庁/金融庁/日本年金機構 before use. Educational information only — **not investment advice** (disclaimer on each screen). Brand swap = 3 `data-k` nodes (`co`/`co2`/`tag`). No tests/lint/build. See `money-basics-quiz/README.md` (Japanese).

### `sourcing-handover/` — 仕入れ引き継ぎコンソール
Zero-dependency single-file vanilla-JS app (no build step — same family as `sales-org-1on1`/`construction-book`) for 株式会社 Martial Arts. Turns the 2026/9/24 仕入れ引き継ぎ (長谷川徹さん → 藪＋CEO) from a paper question sheet into data: answers and 実績 are captured live, then exported as manual-ready Markdown.

- Run: `cd sourcing-handover && python3 -m http.server 5187` → http://localhost:5187 (or open `index.html` directly). `#q` / `#c` / `#o` deep-link the three tabs.
- **Everything lives in `sourcing-handover/index.html`**. Three tabs: ①質問シート (the **61 questions A–G generated from `引き継ぎ質問シート_詳細版_20260924.md`**, per-section + overall progress, filters 全部/最優先ABC/未回答/キーワード) → ②実績カウント (one row per property; auto funnel **出した→徹さんOK→買付提出→審査申込→審査通過→決済済** with 前段比/全体比, 落ちた理由 list, CSV in/out matching `物件出し実績カウント_2026年5月以降.csv`) → ③出力 (manual-ready Markdown of answered items + 未回答 list, JSON backup/restore, print/PDF).
- **Three data tables are the single source of truth**: `QUESTIONS` (id `A-1` form + `sec`; regenerate from the md if the sheet changes), `STAGES` (funnel label + predicate), `COLS` (table column key/label/type — an array type renders a select; CSV headers match the labels).
- Persists to **localStorage only** (`ma_sourcing_handover_v1`), nothing is sent anywhere; 顧客名 is entered as **initials only**. Funnel is a single-series bar chart (one hue, labels outside the fill, hover tooltip) — no legend by design.
- Source material lives in `経営管理/07_業務管理/業務マニュアル/仕入れ・案件管理/` (md + PDF). After the session, feed tab ③'s output back into `物件仕入れマニュアル_再現性版.md` — above all **C = the numeric thresholds for the ★ステップ2 条件表**. No tests/lint/build. See `sourcing-handover/README.md` (Japanese).

### `cashflow-dashboard/` — 資金繰りダッシュボード
Zero-dependency single-file vanilla-JS app (no build step — same family as `construction-book`/`echo-interview-console`). Drop a bank **取引明細 CSV** on the page and it renders 残高推移・月次入出金・月次純収支・カテゴリ別支出・取引先ランキング・資金繰りアラート, plus a filterable full 明細 table. Built against 株式会社 Martial Arts's 楽天銀行 statement, but the column matching is deliberately loose. **All parsing and aggregation happen in the browser — there is no `fetch`/`XHR` anywhere in the file, and nothing is sent anywhere.**

- Run: `cd cashflow-dashboard && python3 -m http.server 5188` → http://localhost:5188 (no install/build); opening `index.html` directly also works. 「サンプルで試す」 generates synthetic data (no PII), so the tool is testable without a real statement.
- **Everything lives in `cashflow-dashboard/index.html`** (markup + CSS + JS). Input columns: `取引日 / 入出金(円) / 残高(円) / 入出金先内容`. **Encoding is auto-detected (Shift_JIS ↔ UTF-8, BOM aware)**; headers are matched fuzzily, and a split 入金/出金 pair works too (a signed single column wins — note `入出金(円)` also substring-matches `出金`, which is why the signed column is resolved first). Multiple CSVs can be dropped and rows are de-duplicated, so months accumulate.
- **Two non-obvious rules make the numbers correct — keep them when editing.** (1) **自社間振替の除外**: transfers to the company's own other account are excluded from 収支/ランキング/カテゴリ (the KPI row shows the net separately), and the self-match runs on `headOf(desc)` — the payee side only — because the `（依頼人名：カ）◯◯）` tail carries the company's own name on *every* outgoing transfer, so matching the whole string flags all of them as internal. (2) **表記ゆれの正規化**: `normKana()` composes 基底＋濁点 (`セソ゛ン`→`セゾン`; NFKC splits a standalone `゛` into 空白＋結合濁点, so the pattern allows that space) and `matchKey()` folds ひらがな→カタカナ, 長音/ハイフン, 小書き→大書き and symbols so `マーシャルアーツ` matches `カ）マ－シヤル　ア－ツ`. Rules and the search box both go through `matchKey`.
- **分類ルールは `DEFAULT_RULES` が既定で、UIから編集可能** (1行 = `カテゴリ , キーワード|…`, top-down first-hit, `__SELF__` = 自社口座キーワード). Unmatched rows fall back to `振込（仕入・外注等）`/`入金（振込）` when the 摘要 looks like a bank transfer (`isTransfer`), else `その他出金`/`その他入金`.
- **Charts are hand-rolled SVG** (no library), colored through CSS custom properties (`--s1` 入金/主系列, `--s2` 出金) that `@media print` swaps to a light palette — the dark surface `#171b21` and the print surface `#ffffff` each use a combination validated for colour-vision separation and contrast, **so re-validate both modes if you change a colour**. Charts redraw at real size via `ResizeObserver` (a stretched `viewBox` would distort the text); direct labels are only drawn when they fit the mark (`fits()`), and カテゴリ別支出 switches its scale to the 2nd-largest category when the largest dwarfs it, saying so on screen. Each chart has a 「表で見る」 table twin.
- **PII discipline**: 明細は既定で保存しない — localStorage persistence is an explicit opt-in toggle (`ma_cashflow_data_v1`, confirm dialog, cleared the moment it is turned off); only settings/rules are always saved (`ma_cashflow_dashboard_v1`). `.gitignore` excludes `cashflow-dashboard/*.csv` — never commit a real statement or anything derived from it. No tests/lint/build. See `cashflow-dashboard/README.md` (Japanese).
- **Deploy (both targets, on push to `main`)**: GitHub Pages via `.github/workflows/pages-construction-book.yml` → `https://hasegawa212.github.io/-/cashflow-dashboard/`, and Netlify (site `hearing-martialarts`) via root `netlify.toml` → `/cashflow-dashboard/`. Publishing the tool is safe because the statement never leaves the browser. **To add another static tool to both, three edits**: the Pages workflow (`paths` + a `cp -r` + a landing card), `netlify/build.sh` (one `cp -R`), and a `no-cache` header block in `netlify.toml`. `netlify/build.sh` assembles `_site` with **echo-interview-console at the root** — that preserves the existing Netlify site's URLs, so never change that first `cp`. `_site/` is gitignored.
