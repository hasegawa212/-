...
- Run: `cd cashflow-dashboard && python3 -m http.server 5187` → http://localhost:5187 (no install/build); opening `index.html` directly also works. 「サンプルで試す」 generates synthetic data (no PII), so the tool is testable without a real statement.
