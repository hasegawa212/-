...
- **`index.html`** is the tool: score each person on **6 axes (0–5)** — ①実績 ②統率 ③育成 ④当事者 ⑤胆力 ⑥定着 — and `recommend()` maps the scores to a role (代表格 / プレイングリーダー / メンター / 管理No.2 / 育成トレーナー / 2番手候補 / プレイヤー) via an ordered priority table. Renders an **org chart + member cards**, add/edit via modal, persists to **localStorage** (`martialarts_1on1_board_v1`), JSON import/export, print/PDF. **5 営業部 members are seeded** as defaults (酒井/上田/小林駿/犬塚篤博/坂本るい; 赤西 is a different company and is excluded; 犬塚 篤博＝アツヒロ is one person — surname 犬塚, given name 篤博).
- **The `recommend()` logic in `index.html` and the 対応表 in `組織設計_来期営業部.md` must stay in sync** — edit both together. Deep-dives: `分析_酒井かいと_20260727.md` (トップセールス), `分析_小林駿_20260724.md` (縁の下の支援スペシャリスト志向＋退職意向＝定着リスク; supersedes the earlier 伝聞「2番手候補 シュン」framing), `分析_坂本るい.md` (係長代理→係長候補・上田課長の右腕/現場リーダー; 自ら昇進を望む).
