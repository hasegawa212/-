...
- **ストリーミング応答**: バックエンドが SSE でトークンを逐次配信し、UI にリアルタイム表示
...
| POST | `/api/chat` | `{ messages, model }` を受け取り、Anthropic の応答を SSE (`text/event-stream`) でストリーム配信する |
...
- **ストリーミング**: 実装済み。`server.js` の `/api/chat` が `messages.stream()` の text_delta を SSE (`event: delta/done/error`) で配信し、`app.js` が逐次表示。チャンク間隔やイベント形式を変えたい場合はこの2ファイルを編集
