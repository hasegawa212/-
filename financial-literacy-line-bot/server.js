...
//
// 構成は japan-mgmt-line-bot / Chat-Bridge の LINE webhook と同じ作り：
...
// ── ヘルスチェック（疎通確認用） ────────────────────────────────
app.get('/', (_req, res) => res.send('financial-literacy-line-bot is running'));
...
// 例: curl 'http://localhost:3000/dev/simulate?text=NISA'
...
app.listen(PORT, () => {
  console.log(`financial-literacy-line-bot listening on http://localhost:${PORT}`);
  console.log(`  webhook: POST /webhook   health: GET /health`);
  if (process.env.NODE_ENV !== 'production') {
    console.log(`  simulate: GET /dev/simulate?text=NISA`);
  }
});
