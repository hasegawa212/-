...
> `/meeting` を**後から追加**する場合（`/daily` 等が既に本番稼働済み）は、手動インポートの代わりに自動スクリプトが使えます。既存ワークフローから credential ID・Sheet ID・Bot Token を自動取得して `slack-modal-trigger-meeting.json` を作成・有効化し、submit-handler に `meeting_check` 分岐を追記します。**n8n に到達できるマシン**で実行してください（Claude Code web 環境からは n8n ホストが許可リスト外で実行不可）:
>
> ```bash
> export N8N_BASE_URL="https://martial-arts-ghd.app.n8n.cloud"
> export N8N_API_KEY="<Settings → n8n API で発行>"
> node n8n-workflows/scripts/import-meeting-workflows.mjs   # DRY_RUN=1 で計画のみ表示
> ```
>
> Node 18+ 必須・リポジトリ直下で実行・冪等（再実行で更新）。実行後は Slack App への `/meeting` コマンド登録だけが残ります。
