...
2. **トリガー絵文字 `furikomi_done` を用意**（※現在ワークスペースに未登録）
   - 方法A（推奨）：Slack の絵文字ピッカー →「絵文字を追加する」→ 任意画像をアップロード →
     名前を **`furikomi_done`**（コロン無し）で保存
   - 方法B：トリガーを既存絵文字に変更する場合は「Parse Slack Event」ノードの
     `const TARGET = 'furikomi_done';` を別の実在絵文字名（例 `moneybag`）に変更
     ※ `white_check_mark`(✅) は完了表示に使用済みなので**トリガーにしない**（無限ループ防止）
3. **テスト投稿の構成**
   - 親メッセージ＝**請求書PDF（.pdfファイル）を添付**（画像はNG）
   - その**スレッド返信**＝振込受付書（スクショ画像 or PDF）
   - 親メッセージに **`furikomi_done`** を付ける（絵文字検索で `furikomi` → 選択）
...
- 実行は出るが一瞬で終わる（数十ms）→「Parse Slack Event」の出力が `skip:true` =
  付けた絵文字が `furikomi_done` でない（絵文字を確認）
- 「Files found?」から「Notify Missing Files」へ進む → 親メッセージに**PDFが無い**
  （または bot が `files:read` 権限/チャンネル参加で添付を読めていない）
